...
MAX(mean_radius) AS max_rad,
MIN(mean_radius) AS min_rad,
...
MAX(surface_gravity) AS max_sg, /** surface_gravity **/
MIN(surface_gravity) AS min_sg  /** surface_gravity **/
FROM solar_system_objects;

SELECT COUNT(mean_radius), body
FROM solar_system_objects
GROUP BY body
HAVING (mean_radius) <= 620; 

SELECT body, type_of_object,
CASE 
WHEN type_of_object LIKE '%satellite%' THEN 'satellite'
WHEN type_of_object LIKE '%asteroid%' THEN 'asteroid'
...
SELECT * 
FROM solar_system_objects
WHERE shape = 'unknown' OR shape = 'round'
ORDER BY shape ASC;
